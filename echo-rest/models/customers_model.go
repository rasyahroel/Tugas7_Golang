...
func FetchCustomers() (res Response, err error) {
	...
	custObj = nil
	for rows.Next() {
		err = rows.Scan(&cust.CustomerID, &cust.CompanyName, &cust.ContactName, &cust.ContactTitle, &cust.Address, &cust.City, &cust.Country, &cust.Phone, &cust.PostalCode)
		if err != nil {
...
func StoreCustomer(e echo.Context) (res Response, err error) {
	...
	con := db.CreateCon()
	sqlStatement := `INSERT INTO customers (CustomerID,CompanyName,ContactName,ContactTitle,Address,City,Country,Phone,PostalCode) VALUES (?,?,?,?,?,?,?,?,?)`
	...
	result, err := stmt.Exec(req.CustomerID, req.CompanyName, req.ContactName, req.ContactTitle, req.Address, req.City, req.Country, req.Phone, req.PostalCode)
